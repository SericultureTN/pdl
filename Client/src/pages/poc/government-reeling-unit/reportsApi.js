...
const UNIT_TYPE = 'government_reeling';
...
/** Downloads the 3-sheet .xlsx for a single already-saved report. */
export async function exportReportExcel(id, filename = 'government-reeling-report.xlsx') {
  const response = await fetch(`${API_BASE}/reports/${id}/export/excel`, {
    credentials: 'include',
  });
  if (!response.ok) await parseApiError(response, 'Failed to export report');
  const blob = await response.blob();
  triggerBlobDownload(blob, filename);
}

/** Admin/secondary_admin only: one workbook covering every office for a month. */
export async function exportAllOfficesExcel({ fiscalYear, month }, filename = 'government-reeling-all-offices.xlsx') {
  const params = new URLSearchParams({ unitType: UNIT_TYPE, officeId: 'all', fiscalYear, month });
  const response = await fetch(`${API_BASE}/reports/export/excel?${params}`, {
    credentials: 'include',
  });
  if (!response.ok) await parseApiError(response, 'Failed to export reports');
  const blob = await response.blob();
  triggerBlobDownload(blob, filename);
}
