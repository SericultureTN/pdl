...
/**
 * Region -> Office picker for Government Reeling Unit's own 8-office list —
 * a flat table (no separate regions table), so distinct regions are derived
 * client-side from the one office list. Controlled: the parent owns
 * region/officeId and receives both back together via onChange, same
 * "changing Region clears Office" contract as RegionMarketOfficeSelect,
 * which this deliberately mirrors visually even though the data source
 * (and region being a plain string, not an id) is different.
 */
export default function GovtReelingOfficeSelect({
  region,
  officeId,
  onChange,
  disabled = false,
